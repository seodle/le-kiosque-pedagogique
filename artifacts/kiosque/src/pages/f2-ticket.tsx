...
export default function F2Ticket() {
  ...
  const isMyTicket = ticket?.status === "assigned_n1";
  const isNew = ticket?.status === "new";
  const isFollowUp = ticket && ["escalated", "assigned_n2", "closed_webex"].includes(ticket.status);
  const canReassign = ticket
    && userId
    && ticket.assignedN1Id === userId
    && ["assigned_n1", "escalated", "assigned_n2", "closed_webex"].includes(ticket.status);
  const showChat = ticket && (isMyTicket || isFollowUp) && !isChatClosed(ticket.status);
      ...
      <div className="max-w-3xl mx-auto py-6 space-y-6">
            ...
            {isMyTicket && (
              <>
                ...
                <Dialog open={escalateOpen} onOpenChange={setEscalateOpen}>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Remonter vers une personne ressource externe</DialogTitle>
                      <DialogDescription>
                        Cette demande sera transmise aux personnes ressources externes disponibles.
                      </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
        ...
        <Card className="border-2">
          <CardHeader className="pb-3">
            ...
            {ticket?.status === "closed_webex" && (
              <p className="text-sm text-muted-foreground font-normal">
                Session visio programmée — vous pouvez continuer à échanger avec l&apos;enseignant.
              </p>
            )}
          </CardHeader>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
              ...
              {messages?.map((msg) => (
                <ChatMessage
                  ...
                  staffLabel="Personne ressource établissement"
                />
              ))}
